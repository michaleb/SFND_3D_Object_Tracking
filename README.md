...
In this project I use keypoint detectors, descriptors, and methods to match them between successive images. Detect objects in an image using the YOLO deep-learning framework and associate regions in a camera image with Lidar points in 3D space in an effort to calculate time-to-collision (TTC) of an ego car with the vehicle directly in front of it.

<img src="images/key_images1.png" width="840" height="320" />
...
3. For camera, associate keypoint matches to regions of interest then to compute TTC based on those matches. 
4. Conduct various tests with the framework to identify the most suitable detector/descriptor combination for TTC estimation and identify problems that can lead to faulty measurements by the camera or Lidar sensor. 
